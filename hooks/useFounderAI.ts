...
export function useFounderAI(profile: UserProfile): UseFounderAIReturn {
    ...
    const handleSuggestDeckFocus = useCallback(async () => {
        ...
        setError(null);
        try {
             const context = `Startup: ${profile.startup.name}. Industry: ${profile.startup.industry}. Stage: ${profile.startup.stage}. Traction: ${profile.startup.traction}. Description: ${profile.startup.description}`;
