...
import { 
  ...
  Plus, Trash2, Lightbulb, MapPin, Award, Search
} from 'lucide-react';
  ...
  <div className="animate-fade-in-up space-y-6">
    ...
    <WizardCard>
      <div className="space-y-8">
        ...
        <div>
          <Label>Current Needs (What are you looking for?)</Label>
          <div className="flex flex-wrap gap-3 mt-2">
            {['Seed Funding', 'Technical Co-founder', 'Beta Testers', 'Enterprise Customers', 'Advisors', 'Backend Engineers', 'Design Partners', 'Sales Partner'].map((need) => (
              <SelectChip 
                key={need} 
                label={need} 
                selected={data.lookingFor.includes(need)}
                onClick={() => {
                  const current = data.lookingFor;
                  update('lookingFor', current.includes(need) ? current.filter(c => c !== need) : [...current, need]);
                }}
                multi
              />
            ))}
...
const ScreenReview = ({ data, setStep }: { data: WizardState; setStep: (s: number) => void }) => {
  const sections = [
    ...
    { title: 'Funding & Needs', step: 7, content: [data.raising ? `Raising $${data.raiseAmount}` : 'Not raising', `Needs: ${data.lookingFor.join(', ')}`] },
  ];
...
const StartupWizard: React.FC = () => {
  ...
  const [wizardData, setWizardData] = useState<WizardState>({
    ...
    raiseAmount: 500000,
    useOfFunds: [],
    lookingFor: profile.lookingFor || [], // Initialize from profile
  ...
  const handleNext = () => {
    if (step === totalSteps - 1) {
        ...
        updateProfile({
            ...
            lookingFor: wizardData.lookingFor, // Add lookingFor
