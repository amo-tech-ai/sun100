...
const DeckCard: React.FC<{ deck: Deck }> = ({ deck }) => (
    <Link to={`/pitch-decks/${deck.id}/edit`} className="block bg-white rounded-lg border border-gray-200/80 shadow-sm overflow-hidden group hover:shadow-md transition-all">
        <div className="aspect-video bg-gray-100 relative">
            {/* Placeholder for first slide image */}
        ...
        <div className="p-4">
            <h3 className="font-bold text-brand-blue truncate group-hover:text-brand-orange transition-colors">{deck.title}</h3>
            <div className="flex justify-between items-center mt-2">
                <p className="text-xs text-gray-500">{deck.slides?.length || 0} slides</p>
                <p className="text-xs text-gray-400">{new Date(deck.updatedAt || Date.now()).toLocaleDateString()}</p>
            </div>
        </div>
    </Link>
);
