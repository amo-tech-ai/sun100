...
import { Deal, getAllDeals, getInteractions, getCustomers, Customer, updateDeal } from '../../services/crmService';
...
export const DealBoard: React.FC = () => {
    ...
    const handleScoreDeal = async (deal: Deal) => {
        if (!deal.accountId) {
            alert("Cannot score deal without an associated customer account.");
            return;
        }
        
        const customer = customers.find(c => c.id === deal.accountId);
        if (!customer) {
            alert("Associated customer not found.");
            return;
        }
                        ...
                        <div className="flex-1 p-2 space-y-3 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-300">
                            {stageDeals.map(deal => (
                                <div 
                                    ...
                                    className={`bg-white p-4 rounded-lg shadow-sm border border-gray-200 hover:shadow-md hover:border-brand-orange/30 transition-all group cursor-grab active:cursor-grabbing ${draggedDealId === deal.id ? 'opacity-50' : ''}`}
                                >
                            ...
                            {stageDeals.length === 0 && (
                                <div className="h-32 flex flex-col items-center justify-center border-2 border-dashed border-gray-200 rounded-lg m-2 text-gray-400">
