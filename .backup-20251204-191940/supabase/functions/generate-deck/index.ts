...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Validate API key
    // According to Supabase docs: https://supabase.com/docs/guides/functions/secrets
    // Secrets are accessed via Deno.env.get('NAME_OF_SECRET')
    // They're available immediately after setting (no redeploy needed)
    const apiKey = Deno.env.get('GEMINI_API_KEY');
    
    // Debug logging (check Dashboard logs to see this output)
    // Docs: https://supabase.com/docs/guides/troubleshooting/inspecting-edge-function-environment-variables-wg5qOQ
    const allEnvKeys = Object.keys(Deno.env.toObject());
    console.log('=== GEMINI_API_KEY DEBUG (from Supabase docs) ===');
    console.log('Deno.env.get("GEMINI_API_KEY"):', apiKey ? `${apiKey.substring(0, 15)}...` : 'undefined');
    console.log('API Key exists:', !!apiKey);
    console.log('API Key length:', apiKey?.length || 0);
    console.log('All env keys count:', allEnvKeys.length);
    console.log('Keys containing "GEMINI":', allEnvKeys.filter(k => k.includes('GEMINI')));
    console.log('Keys containing "SUPABASE":', allEnvKeys.filter(k => k.includes('SUPABASE')));
    console.log('First 15 env keys:', allEnvKeys.slice(0, 15));
    console.log('===============================================');
    
    if (!apiKey || apiKey.trim().length === 0) {
      console.error('ERROR: GEMINI_API_KEY is not accessible');
      console.error('Troubleshooting steps:');
      console.error('1. Verify secret is set: supabase secrets list');
      console.error('2. Check Dashboard: Settings > Edge Functions > Secrets');
      console.error('3. Secret name must match exactly: GEMINI_API_KEY (case-sensitive)');
      console.error('4. Wait 2-3 minutes after setting secret (propagation delay)');
      console.error('5. Check logs: Dashboard > Functions > generate-deck > Logs');
      throw new Error('GEMINI_API_KEY environment variable is not configured. Check Supabase Dashboard logs for debug output.');
    }
    ...
    try {
      ...
      // Handle 403 PERMISSION_DENIED - API key exists but lacks permissions
      if (apiError.status === 403 || apiError.message?.includes('PERMISSION_DENIED')) {
        const errorMsg = apiError.message || '';
        if (errorMsg.includes('suspended')) {
          throw new Error('Gemini API key has been suspended. Please generate a new API key in Google Cloud Console.');
        }
        if (errorMsg.includes('permission')) {
          throw new Error('Gemini API key lacks permissions. Enable "Generative Language API" in Google Cloud Console > APIs & Services > Enabled APIs.');
        }
        throw new Error(`Gemini API permission denied (403). Check: 1) Enable "Generative Language API" in Google Cloud Console, 2) Verify API key restrictions, 3) Ensure API key is active. Error: ${errorMsg}`);
      }
      
      // Handle 401 - Invalid API key
      if (apiError.status === 401 || apiError.message?.includes('API key') && apiError.status !== 403) {
